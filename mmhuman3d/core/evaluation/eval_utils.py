...
def keypoint_mpjpe(pred, gt, mask, alignment='none'):
    ...
    error_array = np.linalg.norm(pred - gt, ord=2, axis=-1)
    error = error_array[mask].mean()
...
def vertice_pve(pred_verts, target_verts, alignment='none'):
    ...
    if alignment == 'none':
        ...
        raise ValueError(f'Invalid value for alignment: {alignment}')
    error_array = np.linalg.norm(pred_verts - target_verts, ord=2, axis=-1)
    error = np.nanmean(error_array)
    return error, error_array

...
from blade.utils.frame_utils import images_to_array
from mmhuman3d.utils.path_utils import check_path_suffix

from blade.cameras.pytorch3d_wrapper.cameras import NewCamerasBase
from .base_renderer import BaseRenderer
from .builder import build_renderer
from blade.render.explicit.pytorch3d_wrapper.lights.lights import (
...
class SMPLRenderer(BaseRenderer):
    ...
    def __init__(self,
        ...
        if output_path is not None:
            ...
            elif check_path_suffix(output_path, ['.png']):
                self.temp_path = None
            else:
        ...
        if plot_pcs:
            self.alpha = max(min(0.9, alpha), 0.5)
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.output_path is not None or self.return_tensor:
            ...
            output_array = tensor2array(output_images)
            ...
            for frame_idx, real_idx in enumerate(indexes):
                folder = self.temp_path if self.temp_path is not None else\
                    self.output_path
                im = output_array[frame_idx]
                if self.plot_pcs and self.vis_kp_index:
                ...
                if self.output_path is not None:
                    if self.output_path.endswith('.png'):
                        ...
                        cv2.imwrite(
                            ...
                            im)
                returned_array.append(im[None])
            returned_array = np.concatenate(returned_array)

...
class IUVShader(nn.Module):
    ...
    def forward(self, fragments, meshes, verts_iuv, **kwargs) -> torch.Tensor:
        """Sample without light."""
        faces = meshes.faces_packed()  # (F, 3)
        verts_iuv = padded_to_packed(verts_iuv)
        faces_iuv = verts_iuv[faces]

        iuv_map = interpolate_face_attributes(
            pix_to_face=fragments.pix_to_face,
            barycentric_coords=fragments.bary_coords,
            face_attributes=faces_iuv)
        return iuv_map[..., 0, :]
...
class SegmentationShader(nn.Module):
    ...
    def forward(self, fragments, meshes, **kwargs) -> torch.Tensor:
        """Sample without light."""
        segmentation_map = meshes.sample_textures(fragments)

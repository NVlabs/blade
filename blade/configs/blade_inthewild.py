...
from blade.configs.base import (root, body_model_train, test_dict_smplx,
                                body_model_test, wandb_init_args, spec_mtp_p3_smplx,
                                itw_dataset)
...
_base_ = ['base.py']
evaluation = dict(metric=['pa-mpjpe', 'mpjpe', 'pve', 'z_error',
...
optimizer = dict(
    pose_backbone=dict(type='AdamW', lr=2e-4, betas=(0.9, 0.999), weight_decay=0),
)
...
model = dict(
    ...
    joint_loss_weight=5.,
    vert_loss_weight=5.,
    # pretrained_depth_backbone_ckpt=f'{root}/pretrained/depth_anything_v2_metric_hypersim_vitl.pth',
    # depthnet_ckpt_path=f'{root}/trained_networks/dav2_ourdata_randcrop_oldandourdata_8x8_mbs2/epoch_4.pth',
    ...
    do_stage_1=False,
    opt_pose=True,      # if True, optimizes pose for improved 2D alignment
    opt_tz=True,
    clear_background=True,    # if True, will extract seg mask directly from the input image
    convert_to_smpl=False,
    n_optimization_iterations=100,
    render_and_save_imgs=True,
    render_gt_instead=False,
    temp_output_folder=f'{root}/results/',
    enable_vis_window=False
)

...
from pytorch_transformers.modeling_bert import (BertPreTrainedModel,
                                                ...
                                                BertSelfOutput)
from blade.models.layers.graph_cnn import GraphResBlock
...
class GraphormerLayer(nn.Module):
    ...
    def __init__(self, config):
        ...
        if self.has_graph_conv == True:
            self.graph_conv = GraphResBlock(config.hidden_size,
                                            config.hidden_size,
                                            config.mesh_sampler.adjmat)
    ...
    def forward(self,
        ...
        return self.MHA_GCN(hidden_states, attention_mask, head_mask,
                            history_state)
...
class Graphormer(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.cls_head = nn.Linear(config.hidden_size,
                                  self.config.output_feature_dim)
        self.residual = nn.Linear(config.img_feature_dim,
                                  self.config.output_feature_dim)
        self.apply(self._init_weights)

...
class Transformer(nn.Module):
    ...
    def __init__(self,
        ...
        # transformer encoder
        encoder_layer = TransformerEncoderLayer(model_dim, nhead,
        ...
        # transformer decoder
        decoder_layer = TransformerDecoderLayer(model_dim, nhead,
                                                feedforward_dim, dropout,
                                                activation)
        decoder_norm = nn.LayerNorm(model_dim)
        self.decoder = TransformerDecoder(decoder_layer, num_dec_layers,
                                          decoder_norm)
    ...
    def forward(self,
        ...
        mask = torch.zeros((bs, hw), dtype=torch.bool,
                           device=device)  # batch_size X (height * width)

        # Transformer Encoder
        zero_mask = torch.zeros((bs, 1), dtype=torch.bool,
                                device=device)  # batch_size X 1
        mem_mask = torch.cat([zero_mask, mask],
                             dim=1)  # batch_size X (1 + height * width)
        cam_with_img = torch.cat(
            [cam_token, img_features],
            dim=0)  # (1 + height * width) X batch_size X feature_dim
        e_outputs = self.encoder(
            cam_with_img, src_key_padding_mask=mem_mask,
            pos=pos_embed)  # (1 + height * width) X batch_size X feature_dim
...
class TransformerEncoderLayer(nn.Module):
    ...
    def __init__(self,
                 ...
                 activation="relu"):
        super().__init__()
        self.self_attn = nn.MultiheadAttention(model_dim,
                                               nhead,
                                               dropout=dropout)
    ...
    def with_pos_embed(self, tensor, pos: Optional[Tensor]):
        ...
        return tensor if pos is None else torch.cat(
            [tensor[:1], (tensor[1:] + pos)], dim=0)
...
class TransformerDecoderLayer(nn.Module):
    ...
    def __init__(self,
        ...
        self.self_attn = nn.MultiheadAttention(model_dim,
                                               nhead,
                                               dropout=dropout)
        self.multihead_attn = nn.MultiheadAttention(model_dim,
                                                    nhead,
                                                    dropout=dropout)

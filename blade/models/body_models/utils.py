...
import torch
from blade.cameras.convert_convention import (
    convert_camera_matrix,
    convert_world_view,
)
from smplx.lbs import batch_rigid_transform, batch_rodrigues
from blade.cameras.utils import combine_RT
from blade.cameras.convert_projection import \
...
from blade.models.body_models.mappings import JOINTS_FACTORY, flip_keypoints_mapping

from blade.utils.torch_utils import cat_pose_list

from blade.models.body_models.mappings import (MANO_JOINTS_LEFT,
                                               MANO_JOINTS_RIGHT, SMPLH_JOINTS,
                                               SMPLH_KEYPOINTS)
from blade.transforms.transform3d import flip_rotation
from blade.models.body_models import get_model_class
import torch
import torch.nn.functional as F
from smplx.utils import find_joint_kin_chain

from blade.models.body_models.mappings import (
    get_keypoint_idx,
    get_keypoint_idxs_by_part,
)
from mmhuman3d.utils.geometry import weak_perspective_projection
from mmhuman3d.models.utils.SMPLX import (
    get_crop_info, find_joint_global_rotation, find_joint_kin_chain,
    flip_rotmat, CropSampler, concat_images, SMPLXFaceCropFunc,
    SMPLXHandCropFunc, SMPLXFaceMergeFunc, SMPLXHandMergeFunc)
from blade.utils.keypoint_utils import get_max_preds_soft
from blade.transforms.transform3d import batch_rodrigues_vectors
from blade.transforms.transform3d import transform_transl
...
def get_crop_info_heatmap(heatmap,
    ...
    center = (torch.einsum(
        ...
        [inv_crop_transforms[:, :2, :2],
         center_body_crop.squeeze(1)]) + inv_crop_transforms[:, :2, 2])
...
class SMPLXHandTranslMergeFunc():
    ...
    def __call__(self, pred_z, pred_body, pred_lhand, pred_rhand, pred_pose,
                 focal_length):
        """Function
        Args:
        ...
        pred_z_lhand = focal_length * torch.exp(
            -pred_lhand['pred_log_s'][:, self.left_wrist_idx])
        pred_transl_lhand = torch.cat(
            [pred_lhand['pred_cam'[:, 1:]], pred_z_lhand], -1)
        pred_z_rhand = focal_length * torch.exp(
            -pred_rhand['pred_log_s'][:, self.right_wrist_idx])
        pred_transl_rhand = torch.cat(
            [pred_rhand['pred_cam'[:, 1:]], pred_z_rhand], -1)
        ...
        model_output = self.body_model(global_orient=global_orient,
                                       body_pose=body_pose,
                                       transl=full_transl)
        left_elbow_transl = model_output['joints'][:, self.left_elbow_idx]
        right_elbow_transl = model_output['joints'][:, self.right_elbow_idx]

        left_elbow_abs = batch_rodrigues_vectors(left_elbow_transl,
                                                 left_wrist_transl)
        right_elbow_abs = batch_rodrigues_vectors(right_elbow_transl,
                                                  right_wrist_transl)
        ...
        pred_pose['body_pose'][:,
                               self.right_elbow_idx - 1] = right_elbow_relative
        pred_pose['body_pose'][:,
                               self.left_elbow_idx - 1] = left_elbow_relative
...
class SMPLXHandHeatmapCropFunc(SMPLXHandCropFunc):
    ...
    def __init__(self,
        ...
        self.lhand_center_idx = get_keypoint_idx(lhand_center_joint,
                                                 convention_body)
        self.rhand_center_idx = get_keypoint_idx(rhand_center_joint,
                                                 convention_body)
    ...
    def __call__(self, body_predictions, img_metas):
        ...
        for img_meta in img_metas:
            full_images.append(
                img_meta['ori_img'].to(device=pred_heatmap.device))
        full_imgs = concat_images(full_images)

        left_hand_points_to_crop = get_crop_info_heatmap(
            pred_heatmap[:, self.lhand_center_idx].unsqueeze(1),
        ...
        # right hand
        right_hand_points_to_crop = get_crop_info_heatmap(
            pred_heatmap[:, self.rhand_center_idx].unsqueeze(1),
...
class SMPLXFaceHeatmapCropFunc(SMPLXFaceCropFunc):
    ...
    def __init__(self,
        ...
        self.face_center_idx = get_keypoint_idx(face_center_joint,
                                                convention_body)
    ...
    def __call__(self, body_predictions, img_metas):
        ...
        face_points_to_crop = get_crop_info_heatmap(
            pred_heatmap[:, self.face_center_idx].unsqueeze(1),
...
if __name__ == '__main__':
    ...
    for index in [100]:
        ...
        from blade.utils.mesh_utils import get_smpl_pc_mesh, get_joints_mesh, save_meshes_as_plys, get_smpl_mesh

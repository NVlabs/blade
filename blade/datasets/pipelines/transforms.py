...
from blade.models.body_models.mappings import get_flip_pairs, convert_kps
...
class GetRandomScaleRotation:
    ...
    def __init__(self, rot_factor=30, scale_factor=0.25, rot_prob=0.6, scale_add=0.0):
    ...
    def __call__(self, results):
        ...
        s_factor = np.clip(np.random.randn() * sf + 1 + self.scale_add, 1 - sf, 1 + sf + self.scale_add)
...
class GetBboxInfo(object):
    ...
    def __init__(
            self,
            rand_shift=0.0, # relative to bbox size
            rand_shift_prob=0.0,  # relative to bbox size
    ):
    ...
    def __call__(self, results):
        ...
        if np.random.rand() < self.rand_shift_prob and self.rand_shift > 0.0:

            shift_x = (np.clip(np.random.randn(), -1., 1.)
                                     * self.rand_shift * results['scale'][0])
            shift_y = (np.clip(np.random.randn(), -1., 1.)
                                     * self.rand_shift * results['scale'][1])
            results['center'][0] += shift_x
            results['center'][1] += shift_y

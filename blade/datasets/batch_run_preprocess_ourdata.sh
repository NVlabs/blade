...
HOME_DIR=~
__conda_setup="$($(HOME_DIR)/miniconda3/bin/conda 'shell.bash' 'hook' 2> /dev/null)"
if [ $? -eq 0 ]; then
    eval "$__conda_setup"
else
...
unset __conda_setup
# <<< conda initialize <<<
conda deactivate; conda activate blade
...
if [ $# -ne 4 ]; then
  echo "Usage: $0 <conda env> <machine_rank> <num_processes_per_machine> <num_sequences_per_process>"
  exit 1
fi
...
run_process() {
  ...
  CMD="taskset -c $CPU python preprocess_ourdata_chunked.py $PROCESS_ID $SEQ_START $SEQ_END"
...
for ((i=0; i<NUM_PROCESSES_PER_MACHINE; i++)); do
  ...
  run_process $CPU $PROCESS_ID $SEQ_START $SEQ_END
done

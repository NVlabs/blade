...
def convert_ndc_to_screen(
    ...
    if isinstance(resolution, (int, float)):
        ...
        resolution = resolution.reshape(-1, 2).to(K.device)
        h_dst, w_dst = resolution[:, 0], resolution[:, 1]

        res = torch.cat([w_dst.reshape(-1, 1), h_dst.reshape(-1, 1)], 1)
        res = torch.min(res, 1)[0].view(-1)
    elif isinstance(resolution, np.array):
...
def convert_screen_to_ndc(
    ...
    if isinstance(resolution, (int, float)):
        ...
        res = torch.cat([w_src.reshape(-1, 1), h_src.reshape(-1, 1)], 1)
        res = torch.min(res, 1)[0].view(-1)

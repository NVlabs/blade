...
import numpy as np
import torch
from blade.models.body_models.mappings import KEYPOINTS_FACTORY
...
def get_max_preds_soft_3d(heatmaps, beta=1e3):
    ...
    heatmaps = torch.softmax(beta * heatmaps,
                             dim=2)  # apply softmax along D*H*W axis
    heatmaps = heatmaps.view(bs, num_j, d, h, w)  # B x J x D x H x W

    # create coordinate tensors
    coords_d = torch.linspace(-1,
                              1,
                              d,
                              device=heatmaps.device,
                              dtype=heatmaps.dtype)
    ...
    coords_w = torch.linspace(-1,
                              ...
                              device=heatmaps.device,
                              dtype=heatmaps.dtype)
    coords_d = coords_d.view(1, 1, -1, 1, 1).expand(bs, num_j, -1, h,
                                                    w)  # B x J x D x H x W
    coords_h = coords_h.view(1, 1, 1, -1, 1).expand(bs, num_j, d, -1,
                                                    w)  # B x J x D x H x W
    coords_w = coords_w.view(1, 1, 1, 1, -1).expand(bs, num_j, d, h,
                                                    -1)  # B x J x D x H x W
...
def get_max_preds_soft_1d(heatmaps, beta=1e3):
    ...
    heatmaps = torch.softmax(beta * heatmaps,
                             dim=2)  # apply softmax along W axis

...
def kp2d_to_bbox_ratiobased(
    ...
    w = (x2 - x1)[0,0]
    h = (y2 - y1)[0,0]
    side = max(min(h,w) * 1.6, max(h,w))/2
    cx = (x2 + x1) / 2
    cy = (y2 + y1) / 2
    bbox = torch.cat([cx - side, cy - side, cx + side, cy + side], -1) if isinstance(
        kp2d, torch.Tensor) else np.concatenate([x1, y1, x2, y2], -1)
    # bbox = np.array([cx - side, cy - side, cx + side, cy + side])[None]
    assert bbox.shape == (num_frame, 4)
    if scale_factor != 1.0:
        bbox = scale_bbox(bbox, scale_factor)
    if xywh:

...
def visualize_kp2d(
    ...
    if mask is not None:
        ...
        assert mask.shape[1] == len(
            keypoints_factory[data_source]
        ), f'mask length should fit with keypoints number \
            {len(keypoints_factory[data_source])}, get {mask.shape}'
    ...
    for frame_index in tqdm(range(kp2d.shape[0]), disable=disable_tqdm):
        ...
        if disable_limbs:
            ...
            limbs_target, limbs_palette = _prepare_limb_palette(
                limbs, palette, pop_parts, data_source, mask_frame,
                keypoints_factory)

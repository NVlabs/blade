...
class PretrainVisualizationHook(Hook):
    ...
    def _draw_samples(self,
        ...
        # prepare images
        images = data_batch['inputs']  ## B x 3 x H x W, not normalized in BGR format
        patch_images = model.head.patchify(images.float()) ## B x (H*W) x (P*P*3)

        ## get the norm_pix constants from the images
        processed_data_batch = model.data_preprocessor(data_batch, True)
        processed_images = processed_data_batch['inputs'] ## B x 3 x H x W, normalized in RGB format.

...
train_pipeline = [
    dict(type='LoadImage'),
    dict(type='RandomBackground', prob=0.5, background_images_root='/uca/sstk/vqa_non_human'), ## for background augmentation
    dict(
        type='RandomResize',
        scale=(768, 1024), ## width, height
        ratio_range=(0.2, 2.0), ## randomly scale image
        keep_ratio=True),
    dict(type='RandomNormalResizeCompensate'), ## compensate for the random resize augmentation
    dict(type='RandomNormalCrop', crop_size=(1024, 768)), ## height, width.

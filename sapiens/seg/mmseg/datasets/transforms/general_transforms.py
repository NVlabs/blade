...
class GeneralRandomFlip(MMCV_RandomFlip):
    def _flip(self, results: dict) -> None:
        ...
        if 'gt_val' in results.keys():
            gt_val_flipped = mmcv.imflip(results['gt_val'], direction=results['flip_direction'])
            results['gt_val'] = gt_val_flipped

@TRANSFORMS.register_module()
class RandomGeneralResizeCompensate(BaseTransform):
    ...
    def transform(self, results: dict) -> dict:
        ...
        if 'gt_val' in results.keys():
            gt_val = results['gt_val']

            ## keep_ratio is true from RandomResize in the config
            if keep_ratio:
                gt_val_resized, scale_factor = mmcv.imrescale(
                    gt_val,
                    results['scale'],
                ...
                gt_val_resized, w_scale, h_scale = mmcv.imresize(
                    gt_val,
                    results['scale'],
                    interpolation='bilinear',
                    return_scale=True,
            ...
            results['gt_val'] = gt_val_resized

            assert results['gt_val'].shape[0] == results['img'].shape[0] and results['gt_val'].shape[1] == results['img'].shape[1]
...
@TRANSFORMS.register_module()
class RandomGeneralCrop(BaseTransform):
...
@TRANSFORMS.register_module()
class GeneralResize(BaseTransform):
...
@TRANSFORMS.register_module()
class GeneralRandomRotate(BaseTransform):
    def __init__(self,
                 prob,
                 degree,
                 pad_val=0,
                 general_pad_val=1e10,
                 center=None,
                 auto_bound=False):
        self.prob = prob
        ...
        self.general_pad_val = general_pad_val
        self.center = center
    ...
    def transform(self, results: dict) -> dict:
        ...
        if rotate:
            ...
            if 'gt_val' in results.keys():
                results['gt_val'] = mmcv.imrotate(
                    results['gt_val'],
                    angle=degree,
                    border_value=self.general_pad_val,
                    center=self.center,
                    auto_bound=self.auto_bound,
                    interpolation='nearest')
    ...
    def __repr__(self):
        ...
        repr_str += f'(prob={self.prob}, ' \
                    ...
                    f'general_pad_val={self.general_pad_val}, ' \
                    f'center={self.center}, ' \
                    f'auto_bound={self.auto_bound})'
        return repr_str

@TRANSFORMS.register_module()
class GenerateGeneralTarget(BaseTransform):
    ...
    def transform(self, results: dict) -> dict:
        gt_val = results['gt_val']
        mask = results['mask']
        gt_val[mask == 0] = self.background_val ## set the background to -1000. the loss uses a threshold of -10 to pick up.
        results['gt_depth_map'] = gt_val
        return results
...
@TRANSFORMS.register_module()
class PackGeneralInputs(BaseTransform):

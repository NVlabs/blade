...
class RandomBackground(BaseTransform):
    ...
    def __init__(
        self,
        prob: float = 0.25,
        background_images_root: str = 'data/coco/no_humans',
    ):
        super().__init__()

        self.prob = prob
        self.background_images_root = background_images_root
        self.background_images = sorted([image_name for image_name in os.listdir(background_images_root) if image_name.endswith('.jpg')])
    ...
    def transform(self, results: dict) -> dict:
        ...
        if LIBCOM_INSTALLED == False:
            warnings.warn(f'libcom not installed, skip random background!')
            return results

        if 'gt_seg_map' in results: 
            gt_seg_map = results['gt_seg_map']
            mask = (gt_seg_map > 0).astype(np.uint8)
        
        elif 'mask' in results:
            mask = results['mask']
            mask = (mask > 0).astype(np.uint8)
        else:
            warnings.warn(f'foreground mask not found in results, skip random background!')
            return results

        background_image_path = os.path.join(self.background_images_root, random.choice(self.background_images))
        ...
        if background_image.shape[0] != image_height or background_image.shape[1] != image_width:
        ...
        if LIBCOM_INSTALLED == True:
            image = color_transfer(pasted_image, composite_mask)
            results['img'] = image ## bgr image

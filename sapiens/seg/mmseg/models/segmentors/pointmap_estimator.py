...
class PointmapEstimator(EncoderDecoder):
    def __init__(self,
                 backbone: ConfigType,
    ...
    def postprocess_result(self,
        ...
        for i in range(batch_size):
            if not only_prediction:
                ...
                i_depth = depth[i:i + 1, :, padding_top:H - padding_bottom, padding_left:W - padding_right]
                ...
                i_depth_X = i_depth[:, 0].unsqueeze(1) ## B x 1 X H x W
                i_depth_Y = i_depth[:, 1].unsqueeze(1) ## B x 1 X H x W
                i_depth_Z = i_depth[:, 2].unsqueeze(1) ## B x 1 X H x W

                i_depth_resized_X = F.interpolate(i_depth_X, size=img_meta['ori_shape'], mode='bilinear', align_corners=self.align_corners)
                i_depth_resized_Y = F.interpolate(i_depth_Y, size=img_meta['ori_shape'], mode='bilinear', align_corners=self.align_corners)
                i_depth_resized_Z = F.interpolate(i_depth_Z, size=img_meta['ori_shape'], mode='bilinear', align_corners=self.align_corners)

                i_depth = torch.cat([i_depth_resized_X, i_depth_resized_Y, i_depth_resized_Z], dim=1) ## B x 3 X H x W
                i_depth = i_depth.squeeze(0) ## 3 X H x W

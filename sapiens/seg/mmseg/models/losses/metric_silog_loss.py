...
class MetricSiLogLoss(nn.Module):
    """Compute SiLog loss.
    ...
    def forward(
        ...
        pred_normalized = self.normalize_depth(pred.clone(), valid_mask, background_val=self.background_val)
        target_normalized = self.normalize_depth(target.clone(), valid_mask, background_val=self.background_val)

...
class SegLocalVisualizer(Visualizer):
    ...
    def _draw_sem_seg(self,
        ...
        if withLabels:
            ...
            scale = 0.05
            # fontScale = min(image.shape[0], image.shape[1]) / (25 / scale) ## very big
            # fontScale = min(image.shape[0], image.shape[1]) / (50 / scale) ## for paper
            fontScale = min(image.shape[0], image.shape[1]) / (75 / scale) ## default for goliath
            # fontScale = min(image.shape[0], image.shape[1]) / (100 / scale) 

...
import cv2
from aios_repo.config.config import cfg

import matplotlib.pyplot as plt

aios_root_dir = osp.dirname(osp.dirname(osp.dirname(osp.abspath(__file__))))
# sys.path.append(aios_root_dir)


class AiOSSMPLX_ControlNet(nn.Module):
    def __init__(
        self,
        backbone,
        transformer,
        backbone_controlnet,
        transformer_controlnet,
        num_classes,
        num_queries,
        aux_loss=False,
        iter_update=True,
        ...
        # backbone controlnet output zero convs
        # self.backbone_zeroconv = nn.ModuleList([nn.Conv2d(512, 512, kernel_size=3, stride=1, padding=1),
        #                                   nn.Conv2d(1024, 1024, kernel_size=3, stride=1, padding=1),
        #                                   nn.Conv2d(2048, 2048, kernel_size=3, stride=1, padding=1)])
        # for layer_i in range(len(self.backbone_zeroconv)):
        #     nn.init.zeros_(self.backbone_zeroconv[layer_i].weight)
        #     nn.init.zeros_(self.backbone_zeroconv[layer_i].bias)
        # self.backbone_poss_zeroconv = nn.ModuleList([nn.Conv2d(256, 256, kernel_size=3, stride=1, padding=1),
        #                                              nn.Conv2d(256, 256, kernel_size=3, stride=1, padding=1),
        #                                              nn.Conv2d(256, 256, kernel_size=3, stride=1, padding=1),
        #                                              nn.Conv2d(256, 256, kernel_size=3, stride=1, padding=1)])
        # for layer_i in range(len(self.backbone_poss_zeroconv)):
        #     nn.init.zeros_(self.backbone_poss_zeroconv[layer_i].weight)
        #     nn.init.zeros_(self.backbone_poss_zeroconv[layer_i].bias)

        # backbone xformer controlnet
        self.transformer_controlnet = transformer_controlnet

        # controlnet input zero convs
        self.Tz_zeromlp = nn.ModuleList([nn.Linear(1, 512), nn.Linear(1, 1024), nn.Linear(1, 2048)])
        for layer_i in range(len(self.Tz_zeromlp)):
            nn.init.zeros_(self.Tz_zeromlp[layer_i].weight)
            nn.init.zeros_(self.Tz_zeromlp[layer_i].bias)
        self.Tz_poss_zeromlp = nn.ModuleList([nn.Linear(1, 256), nn.Linear(1, 256), nn.Linear(1, 256), nn.Linear(1, 256)])
        for layer_i in range(len(self.Tz_poss_zeromlp)):
            nn.init.zeros_(self.Tz_poss_zeromlp[layer_i].weight)
            nn.init.zeros_(self.Tz_poss_zeromlp[layer_i].bias)


        # controlnet output zero convs
        # self.hs_zeroconv_list = nn.ModuleList([nn.Linear(256, 256), nn.Linear(256, 256),
        #                                        nn.Linear(256, 256), nn.Linear(256, 256),
        #                                        nn.Linear(256, 256), nn.Linear(256, 256),])
        # for layer_i in range(len(self.hs_zeroconv_list)):
        #     nn.init.zeros_(self.hs_zeroconv_list[layer_i].weight)
        #     nn.init.zeros_(self.hs_zeroconv_list[layer_i].bias)
        # self.reference_zeroconv_list = nn.ModuleList([nn.Linear(4, 4), nn.Linear(4, 4),
        #                                          nn.Linear(4, 4), nn.Linear(4, 4),
        #                                          nn.Linear(4, 4), nn.Linear(4, 4), nn.Linear(4, 4)])
        # for layer_i in range(len(self.reference_zeroconv_list)):
        #     nn.init.zeros_(self.reference_zeroconv_list[layer_i].weight)
        #     nn.init.zeros_(self.reference_zeroconv_list[layer_i].bias)


        self.trainable_modules.extend([self.transformer_controlnet,
                                       self.Tz_zeromlp, self.Tz_poss_zeromlp,
                                       # self.hs_zeroconv_list, self.reference_zeroconv_list
                                       ])
        ...
        if inference:
            body_model=dict(
                ...
                num_betas=10,
                keypoint_dst='smplx',
                model_path=f'{aios_root_dir}/../body_models/smplx',
                use_pca=False,
                use_face_contour=True)
        ...
        if num_feature_levels > 1:
            ...
            for _ in range(num_backbone_outs):
                ...
                input_proj_list_controlnet.append(
                    nn.Sequential(
                        nn.Conv2d(in_channels, hidden_dim, kernel_size=1),
                        nn.GroupNorm(32, hidden_dim),
                    ))
            ...
            self.input_proj_controlnet = nn.ModuleList(input_proj_list_controlnet)
        else:
            ...
            self.input_proj_controlnet = nn.ModuleList([
                nn.Sequential(
                    nn.Conv2d(backbone.num_channels[-1],
                              hidden_dim,
                              kernel_size=1),
        ...
        self.backbone_controlnet = backbone_controlnet
        self.aux_loss = aux_loss
        self.box_pred_damping = box_pred_damping = None

        self.trainable_modules.extend([self.input_proj_controlnet, self.backbone_controlnet])
        ...
        _class_embed_controlnet = nn.Linear(hidden_dim,
                                 num_classes,
                                 bias=(not cls_no_bias))
        if not cls_no_bias:
            prior_prob = 0.01
            ...
            _class_embed_controlnet.bias.data = torch.ones(self.num_classes) * bias_value
        ...
        if dec_pred_class_embed_share:
            ...
            class_embed_layerlist_controlnet = [
                _class_embed_controlnet for i in range(transformer_controlnet.num_decoder_layers)
            ]
        else:
            class_embed_layerlist = [
            ...
            class_embed_layerlist_controlnet = [
                copy.deepcopy(_class_embed_controlnet)
                for i in range(transformer.num_decoder_layers)
            ]
        ...
        _bbox_embed_controlnet = MLP(hidden_dim, hidden_dim, 4, 3)
        nn.init.constant_(_bbox_embed_controlnet.layers[-1].weight.data, 0)
        nn.init.constant_(_bbox_embed_controlnet.layers[-1].bias.data, 0)
        ...
        if dec_pred_bbox_embed_share:
            ...
            box_body_embed_layerlist_controlnet = [
                _bbox_embed_controlnet for i in range(transformer.num_decoder_layers)
            ]
        else:
            box_body_embed_layerlist = [
            ...
            box_body_embed_layerlist_controlnet = [
                copy.deepcopy(_bbox_embed_controlnet)
                for i in range(transformer.num_decoder_layers)
            ]
        ...
        _bbox_hand_embed_controlnet = MLP(hidden_dim, hidden_dim, 2, 3) # TODO: the out shape should be 2 not 4
        nn.init.constant_(_bbox_hand_embed_controlnet.layers[-1].weight.data, 0)
        nn.init.constant_(_bbox_hand_embed_controlnet.layers[-1].bias.data, 0)
        ...
        _bbox_hand_hw_embed_controlnet = MLP(hidden_dim, hidden_dim, 2, 3)
        nn.init.constant_(_bbox_hand_hw_embed_controlnet.layers[-1].weight.data, 0)
        nn.init.constant_(_bbox_hand_hw_embed_controlnet.layers[-1].bias.data, 0)
        ...
        if dec_pred_pose_embed_share:
            ...
            box_hand_embed_layerlist_controlnet = \
                [_bbox_hand_embed_controlnet for i in range(transformer.num_decoder_layers - num_box_decoder_layers+1)]
        else:
            box_hand_embed_layerlist = [
            ...
            box_hand_embed_layerlist_controlnet = [
                copy.deepcopy(_bbox_hand_embed_controlnet)
                for i in range(transformer.num_decoder_layers -
                            num_box_decoder_layers + 1)
            ]
        ...
        if dec_pred_pose_embed_share:
            ...
            box_hand_hw_embed_layerlist_controlnet = [
                _bbox_hand_hw_embed_controlnet for i in range(
                    transformer.num_decoder_layers - num_box_decoder_layers)
                ]
        else:
            box_hand_hw_embed_layerlist = [
            ...
            box_hand_hw_embed_layerlist_controlnet = [
                copy.deepcopy(_bbox_hand_hw_embed_controlnet)
                for i in range(transformer.num_decoder_layers -
                            num_box_decoder_layers)
            ]
        ...
        _bbox_face_embed_controlnet = MLP(hidden_dim, hidden_dim, 2, 3)
        nn.init.constant_(_bbox_face_embed_controlnet.layers[-1].weight.data, 0)
        nn.init.constant_(_bbox_face_embed_controlnet.layers[-1].bias.data, 0)
        ...
        _bbox_face_hw_embed_controlnet = MLP(hidden_dim, hidden_dim, 2, 3)
        nn.init.constant_(_bbox_face_hw_embed_controlnet.layers[-1].weight.data, 0)
        nn.init.constant_(_bbox_face_hw_embed_controlnet.layers[-1].bias.data, 0)
        ...
        if dec_pred_pose_embed_share:
            ...
            box_face_embed_layerlist_controlnet = [
                _bbox_face_embed_controlnet for i in range(
                    transformer.num_decoder_layers - num_box_decoder_layers + 1)
                ]
        else:
            box_face_embed_layerlist = [
            ...
            box_face_embed_layerlist_controlnet = [
                copy.deepcopy(_bbox_face_embed_controlnet)
                for i in range(transformer.num_decoder_layers -
                            num_box_decoder_layers + 1)
            ]
        ...
        if dec_pred_pose_embed_share:
            box_face_hw_embed_layerlist = [
                _bbox_face_hw_embed for i in range(
                    transformer.num_decoder_layers - num_box_decoder_layers)]
            box_face_hw_embed_layerlist_controlnet = [
                _bbox_face_hw_embed_controlnet for i in range(
                    transformer.num_decoder_layers - num_box_decoder_layers)]
        else:
            box_face_hw_embed_layerlist = [
            ...
            box_face_hw_embed_layerlist_controlnet = [
                copy.deepcopy(_bbox_face_hw_embed_controlnet)
                for i in range(transformer.num_decoder_layers -
                            num_box_decoder_layers)
            ]
        ###########################################################################
        ...
        _pose_embed_controlnet = MLP(hidden_dim, hidden_dim, 2, 3)
        nn.init.constant_(_pose_embed_controlnet.layers[-1].weight.data, 0)
        nn.init.constant_(_pose_embed_controlnet.layers[-1].bias.data, 0)
        ...
        if num_body_points == 17:
            if dec_pred_pose_embed_share:
                ...
                pose_embed_layerlist_controlnet = \
                    [_pose_embed_controlnet for i in range(transformer.num_decoder_layers - num_box_decoder_layers+1)]
            else:
                pose_embed_layerlist = [
                ...
                pose_embed_layerlist_controlnet = [
                    copy.deepcopy(_pose_embed_controlnet)
                    for i in range(transformer.num_decoder_layers -
                                num_box_decoder_layers + 1)
                ]
        else:
            if dec_pred_pose_embed_share:
                ...
                pose_embed_layerlist_controlnet = [
                    _pose_embed_controlnet for i in range(transformer.num_decoder_layers -
                                            num_box_decoder_layers)
                ]
            else:
                pose_embed_layerlist = [
        ...
        _pose_hw_embed_controlnet = MLP(hidden_dim, hidden_dim, 2, 3)
        ...
        pose_hw_embed_layerlist_controlnet = [
            _pose_hw_embed_controlnet for i in range(transformer.num_decoder_layers -
                                        num_box_decoder_layers)
        ]
        ...
        _pose_hand_embed_controlnet = MLP(hidden_dim, hidden_dim, 2, 3)
        nn.init.constant_(_pose_hand_embed_controlnet.layers[-1].weight.data, 0)
        nn.init.constant_(_pose_hand_embed_controlnet.layers[-1].bias.data, 0)
        ...
        if dec_pred_pose_embed_share:
            ...
            pose_hand_embed_layerlist_controlnet = \
                [_pose_hand_embed_controlnet for i in range(transformer.num_decoder_layers - num_hand_face_decoder_layers+1)]
        else:
            pose_hand_embed_layerlist = [
            ...
            pose_hand_embed_layerlist_controlnet = [
                copy.deepcopy(_pose_hand_embed_controlnet)
                for i in range(transformer.num_decoder_layers -
                            num_hand_face_decoder_layers + 1)
            ]
        ...
        _pose_hand_hw_embed_controlnet = MLP(hidden_dim, hidden_dim, 2, 3)
        ...
        pose_hand_hw_embed_layerlist_controlnet = [
            _pose_hand_hw_embed_controlnet for i in range(transformer.num_decoder_layers -
                                        num_hand_face_decoder_layers)
        ]
        ...
        _pose_face_embed_controlnet = MLP(hidden_dim, hidden_dim, 2, 3)
        nn.init.constant_(_pose_face_embed_controlnet.layers[-1].weight.data, 0)
        nn.init.constant_(_pose_face_embed_controlnet.layers[-1].bias.data, 0)
        ...
        if dec_pred_pose_embed_share:
            ...
            pose_face_embed_layerlist_controlnet = \
                [_pose_face_embed_controlnet for i in range(transformer.num_decoder_layers - num_hand_face_decoder_layers+1)]
        else:
            pose_face_embed_layerlist = [
            ...
            pose_face_embed_layerlist_controlnet = [
                copy.deepcopy(_pose_face_embed_controlnet)
                for i in range(transformer.num_decoder_layers -
                            num_hand_face_decoder_layers + 1)
            ]
        ...
        _pose_face_hw_embed_controlnet = MLP(hidden_dim, hidden_dim, 2, 3)
        ...
        pose_face_hw_embed_layerlist_controlnet = [
            _pose_face_hw_embed_controlnet for i in range(transformer.num_decoder_layers -
                                        num_hand_face_decoder_layers)
        ]
        ...
        self.bbox_embed_controlnet = nn.ModuleList(box_body_embed_layerlist_controlnet)
        self.class_embed_controlnet = nn.ModuleList(class_embed_layerlist_controlnet)
        self.pose_embed_controlnet = nn.ModuleList(pose_embed_layerlist_controlnet)
        self.pose_hw_embed_controlnet = nn.ModuleList(pose_hw_embed_layerlist_controlnet)
        self.transformer_controlnet.decoder.bbox_embed = self.bbox_embed_controlnet
        self.transformer_controlnet.decoder.pose_embed = self.pose_embed_controlnet
        self.transformer_controlnet.decoder.pose_hw_embed = self.pose_hw_embed_controlnet
        self.transformer_controlnet.decoder.class_embed = self.class_embed_controlnet

        self.trainable_modules.extend([self.bbox_embed_controlnet, self.class_embed_controlnet,
                                       self.pose_embed_controlnet, self.pose_hw_embed_controlnet])
        ...
        self.bbox_hand_embed_controlnet = nn.ModuleList(box_hand_embed_layerlist_controlnet)
        self.bbox_hand_hw_embed_controlnet = nn.ModuleList(box_hand_hw_embed_layerlist_controlnet)
        self.pose_hand_embed_controlnet = nn.ModuleList(pose_hand_embed_layerlist_controlnet)
        self.pose_hand_hw_embed_controlnet = nn.ModuleList(pose_hand_hw_embed_layerlist_controlnet)
        self.transformer_controlnet.decoder.bbox_hand_embed = self.bbox_hand_embed_controlnet
        self.transformer_controlnet.decoder.bbox_hand_hw_embed = self.bbox_hand_hw_embed_controlnet
        self.transformer_controlnet.decoder.pose_hand_embed = self.pose_hand_embed_controlnet
        self.transformer_controlnet.decoder.pose_hand_hw_embed = self.pose_hand_hw_embed_controlnet

        self.trainable_modules.extend([self.bbox_hand_embed_controlnet, self.bbox_hand_hw_embed_controlnet,
                                       self.pose_hand_embed_controlnet, self.pose_hand_hw_embed_controlnet])
        ...
        self.bbox_face_embed_controlnet = nn.ModuleList(box_face_embed_layerlist_controlnet)
        self.bbox_face_hw_embed_controlnet = nn.ModuleList(box_face_hw_embed_layerlist_controlnet)
        self.pose_face_embed_controlnet = nn.ModuleList(pose_face_embed_layerlist_controlnet)
        self.pose_face_hw_embed_controlnet = nn.ModuleList(pose_face_hw_embed_layerlist_controlnet)
        self.transformer_controlnet.decoder.bbox_face_embed = self.bbox_face_embed_controlnet
        self.transformer_controlnet.decoder.bbox_face_hw_embed = self.bbox_face_hw_embed_controlnet
        self.transformer_controlnet.decoder.pose_face_embed = self.pose_face_embed_controlnet
        self.transformer_controlnet.decoder.pose_face_hw_embed = self.pose_face_hw_embed_controlnet

        self.trainable_modules.extend([self.bbox_face_embed_controlnet, self.bbox_face_hw_embed_controlnet,
                                       self.pose_face_embed_controlnet, self.pose_face_hw_embed_controlnet])
        ...
        self.transformer_controlnet.decoder.num_hand_face_decoder_layers = num_hand_face_decoder_layers
        self.transformer_controlnet.decoder.num_box_decoder_layers = num_box_decoder_layers
        self.transformer_controlnet.decoder.num_body_points = num_body_points
        self.transformer_controlnet.decoder.num_hand_points = num_hand_points
        self.transformer_controlnet.decoder.num_face_points = num_face_points
        # two stage
        ...
        if two_stage_type != 'no':
            if two_stage_bbox_embed_share:
                ...
                self.transformer_controlnet.enc_out_bbox_embed = _bbox_embed_controlnet
            else:
                self.transformer.enc_out_bbox_embed = copy.deepcopy(_bbox_embed)
                self.transformer_controlnet.enc_out_bbox_embed = copy.deepcopy(_bbox_embed_controlnet)

            if two_stage_class_embed_share:
                assert dec_pred_class_embed_share and dec_pred_bbox_embed_share
                self.transformer.enc_out_class_embed = _class_embed
                self.transformer_controlnet.enc_out_class_embed = _class_embed_controlnet
            else:
                self.transformer.enc_out_class_embed = copy.deepcopy(_class_embed)
                self.transformer_controlnet.enc_out_class_embed = copy.deepcopy(_class_embed_controlnet)
    ...
    def body_localization(self, ctx):
        reference, hs = ctx['reference'], ctx['hs']

        # update human boxes
        # effective_dn_number = self.dn_number if self.training else 0
        effective_dn_number = 0
        ...
        for dec_lid, (layer_ref_sig, layer_body_bbox_embed, layer_cls_embed,
            ...
            if dec_lid < self.num_box_decoder_layers:
                ...
                reference_before_sigmoid_body_bbox_norm = layer_ref_sig[:, effective_dn_number:, :][
                                                          :, 0::(self.num_whole_body_points + 4),
                                                          :]  # norm position query
                layer_body_box_delta_unsig_norm = layer_body_bbox_embed(layer_hs_body_bbox_norm)
                layer_body_box_outputs_unsig_norm = layer_body_box_delta_unsig_norm + inverse_sigmoid(
                    reference_before_sigmoid_body_bbox_norm)
        ...
        return outputs_body_bbox_list, outputs_class, layer_cls_norm, effective_dn_number

    def body_refinement(self, ctx):
        reference, hs, layer_cls_norm, effective_dn_number \
            = ctx['reference'], ctx['hs'], ctx['layer_cls_norm'], ctx['effective_dn_number']
        ...
        # smpl rhand
        rhand_index = list(
            range(self.num_body_points + self.num_hand_points + 2, self.num_body_points + self.num_hand_points * 2 + 3))
        ...
        # smpl face
        face_index = list(range(self.num_body_points + self.num_hand_points * 2 + 3,
                                self.num_body_points + self.num_hand_points * 2 + self.num_face_points + 4))
        ...
        for dec_lid, (layer_ref_sig, layer_hs) in enumerate(zip(reference[:-1], hs)):
            if dec_lid < self.num_box_decoder_layers:
                ...
                layer_ref_sig_body_kpt = \
                    layer_ref_sig[:, effective_dn_number:, :].index_select(1, torch.tensor(body_kpt_index,
                ...
                delta_lhand_bbox_xy_unsig = self.bbox_hand_embed[dec_lid - self.num_box_decoder_layers](
                    layer_hs_lhand_bbox)
                ...
                delta_face_bbox_xy_unsig = self.bbox_face_embed[dec_lid - self.num_box_decoder_layers](
                    layer_hs_face_bbox)
                layer_ref_sig_face_bbox = \
                    layer_ref_sig[:, effective_dn_number:, :][
                    :, (self.num_body_points + 2 * self.num_hand_points + 3)::(self.num_whole_body_points + 4),
        ...
        return (outputs_lhand_bbox_list, outputs_rhand_bbox_list, outputs_face_bbox_list,
         outputs_body_keypoints_list, outputs_body_keypoints_hw, outputs_lhand_keypoints_list,
         outputs_lhand_keypoints_hw, outputs_rhand_keypoints_list, outputs_rhand_keypoints_hw,
         outputs_face_keypoints_list, outputs_face_keypoints_hw, outputs_smpl_pose_list,
         outputs_smpl_lhand_pose_list, outputs_smpl_rhand_pose_list, outputs_smpl_expr_list,
         outputs_smpl_jaw_pose_list, outputs_smpl_beta_list, outputs_smpl_cam_list,
         outputs_smpl_kp2d_list, outputs_smpl_kp3d_list, outputs_smpl_verts_list)

    def print_gradients(self, name):
        def hook(grad):
    ...
    def forward(self, data_batch: NestedTensor, targets: List = None):
        ...
        if isinstance(data_batch, dict):
            ...
            samples = data_batch
        samples_controlnet = copy.deepcopy(samples)
        ...
        # controlnet, condition on Tz
        features_controlnet, poss_controlnet = self.backbone_controlnet(samples_controlnet)
        for f_i in range(len(features_controlnet)):
            features_controlnet[f_i].tensors = features_controlnet[f_i].tensors + self.Tz_zeromlp[f_i](data_batch['pred_z'].detach())[..., None, None]
        for f_i in range(len(poss_controlnet)):
            poss_controlnet[f_i] = poss_controlnet[f_i] + self.Tz_poss_zeromlp[f_i](data_batch['pred_z'].detach())[..., None, None]
        ...
        with torch.no_grad():
            ...
            for l, feat in enumerate(features):  # len(features=3)
                ...
                self.input_proj[l].eval()
                srcs.append(self.input_proj[l](src))
                masks.append(mask)
                assert mask is not None
        srcs_controlnet = []
        masks_controlnet = []
        for l, feat in enumerate(features_controlnet):  # len(features=3)
            src_controlnet, mask_controlnet = feat.decompose()
            srcs_controlnet.append(self.input_proj_controlnet[l](src_controlnet))
            masks_controlnet.append(mask_controlnet)
            assert mask_controlnet is not None
        ...
        if self.num_feature_levels > len(srcs):
            ...
            _len_srcs_controlnet = len(srcs_controlnet)
            for l in range(_len_srcs_controlnet, self.num_feature_levels):
                if l == _len_srcs_controlnet:
                    src = self.input_proj_controlnet[l](features_controlnet[-1].tensors)
                else:
                    src = self.input_proj_controlnet[l](srcs_controlnet[-1])
                m = samples_controlnet.mask
                mask = F.interpolate(m[None].float(),
                                     size=src.shape[-2:]).to(torch.bool)[0]
                pos_l = self.backbone_controlnet[1](NestedTensor(src, mask)).to(src.dtype)
                srcs_controlnet.append(src)
                masks_controlnet.append(mask)
                poss_controlnet.append(pos_l)

        if self.dn_number > 0 or targets is not None:
            input_query_label, input_query_bbox, attn_mask, attn_mask2, attn_mask3, mask_dict = \
                self.prepare_for_dn2(targets)
            (input_query_label_controlnet, input_query_bbox_controlnet, attn_mask_controlnet, attn_mask2_controlnet,
             attn_mask3_controlnet, mask_dict_controlnet) = self.prepare_for_dn2(targets)
        else:
            assert targets is None
            input_query_bbox_controlnet = input_query_label_controlnet = attn_mask_controlnet = attn_mask2_controlnet \
                = attn_mask3_controlnet = mask_dict_controlnet = input_query_bbox = input_query_label = attn_mask = attn_mask2 = attn_mask3 = mask_dict = None
        ...
        hs_controlnet, reference_controlnet, _, _, _ = self.transformer_controlnet(
            srcs_controlnet, masks_controlnet, input_query_label_controlnet, poss_controlnet, input_query_label_controlnet, attn_mask_controlnet,
            attn_mask2_controlnet, attn_mask3_controlnet)
        ...
        for dec_lid, (layer_ref_sig, layer_hs, layer_ref_sig_controlnet, layer_hs_controlnet) \
                in enumerate(zip(reference[:-1], hs, reference_controlnet[:-1], hs_controlnet)):
            if dec_lid < self.num_box_decoder_layers:
                if True:
                    assert isinstance(layer_hs, torch.Tensor)
                    bs = layer_hs.shape[0]
                ...
                smpl_feats = layer_hs[:, effective_dn_number:, :].reshape(
                    bs, -1, feat_dim * (self.num_body_points + 4))
                # controlnet
                smpl_feats_controlnet = layer_hs_controlnet[:, effective_dn_number:, :].reshape(
                    bs, -1, feat_dim * (self.num_body_points + 4))
                smpl_feats = smpl_feats + smpl_feats_controlnet
                ...
                # controlnet
                smpl_body_pose_feats_controlnet = layer_hs_controlnet[:, effective_dn_number:, :].index_select(
                    1, torch.tensor(smpl_pose_index, device=layer_hs.device)
                ).reshape(bs, -1, feat_dim * (self.num_body_points + 4))
                smpl_body_pose_feats = smpl_body_pose_feats + smpl_body_pose_feats_controlnet
        ...
        # # log gradients
        # full_pose.register_hook(self.print_gradients("full_pose"))
        # # reference[0].register_hook(self.print_gradients("reference[0]"))
        # reference[1].register_hook(self.print_gradients("reference[1]"))
        # reference[2].register_hook(self.print_gradients("reference[2]"))
        # reference[3].register_hook(self.print_gradients("reference[3]"))
        # reference[4].register_hook(self.print_gradients("reference[2]"))
        # reference[5].register_hook(self.print_gradients("reference[3]"))
        # reference[6].register_hook(self.print_gradients("reference[2]"))
        # hs[0].register_hook(self.print_gradients("hs[0]"))
        # hs[1].register_hook(self.print_gradients("hs[1]"))
        # hs[2].register_hook(self.print_gradients("hs[2]"))
        # hs[3].register_hook(self.print_gradients("hs[0]"))
        # hs[4].register_hook(self.print_gradients("hs[1]"))
        # hs[5].register_hook(self.print_gradients("hs[2]"))
        # outputs_smpl_pose_list[-1].register_hook(self.print_gradients("outputs_smpl_pose_list[-1]"))

        return out, targets, data_batch
        # return out, targets, data_batch, (outputs_lhand_bbox_list, outputs_rhand_bbox_list, outputs_face_bbox_list,
        #                                   outputs_body_keypoints_list, outputs_body_keypoints_hw,
        #                                   outputs_lhand_keypoints_list,
        #                                   outputs_lhand_keypoints_hw, outputs_rhand_keypoints_list,
        #                                   outputs_rhand_keypoints_hw,
        #                                   outputs_face_keypoints_list, outputs_face_keypoints_hw,
        #                                   outputs_smpl_pose_list,
        #                                   outputs_smpl_lhand_pose_list, outputs_smpl_rhand_pose_list,
        #                                   outputs_smpl_expr_list,
        #                                   outputs_smpl_jaw_pose_list, outputs_smpl_beta_list, outputs_smpl_cam_list,
        #                                   outputs_smpl_kp2d_list, outputs_smpl_kp3d_list, outputs_smpl_verts_list)


    def forward_original(self, data_batch: NestedTensor, targets: List = None):
        """The forward expects a NestedTensor, which consists of:
    ...
    def prepare_targets(self, data_batch):
        ...
        for key in data_batch:
            if isinstance(data_batch[key], DataContainer):
                data_batch[key] = data_batch[key]._data[0]

        data_batch_coco = []
        instance_dict = {}
        img_list = data_batch['img'].float().to(device)
        ...
        masks = torch.ones((batch_size, input_img_h, input_img_w),
                           dtype=torch.bool,
                           device=device)
        # plt.imshow(img_list[1].permute(1, 2, 0).detach().cpu() / 3 + 0.5); plt.show()
        ...
        for img_id in range(batch_size):
            ...
            if not self.inference:
                ...
                instance_dict = {}
                # instance_dict['orig_size'] = data_batch['ori_shape'][img_id]
                instance_dict['size'] = data_batch['img_shape'][img_id].to(device)  # after augmentation
                instance_dict['boxes'] = instance_body_bbox.float().to(device)
...
@MODULE_BUILD_FUNCS.registe_with_name(module_name='aios_smplx_controlnet')
def build_aios_smplx_controlnet(args, cfg):
    # pdb.set_trace()
    ...
    backbone_controlnet = build_backbone(args)
    transformer_controlnet = build_transformer(args)

    ...
    def __init__(self,
        ...
        self.body_model = build_body_model(
            dict(type='GenderedSMPL',
                 keypoint_src='h36m',
                 keypoint_dst='h36m',
                 model_path=f'{aios_root_dir}/data/body_models/smpl',
                 keypoint_approximate=True,
                 joints_regressor=
                 f'{aios_root_dir}/data/body_models/J_regressor_h36m.npy'))
...
class PostProcess_SMPLX(nn.Module):
    ...
    def __init__(
        ...
        body_model= dict(
            ...
            model_path=f'{aios_root_dir}/data/body_models/smplx',
            use_pca=False,
...
class PostProcess_SMPLX_Multi_Infer(nn.Module):
    ...
    def forward_withgrad(self, outputs, target_sizes, targets, data_batch_nc, image_shape=None, not_to_xyxy=False, test=False):
        """
        ...
        for bs in range(batch_size):
            ...
            out_smpl_kp2d.append(out_kp2d_i)
        out_smpl_kp2d = torch.stack(out_smpl_kp2d, dim=0).detach()
        ...
        smpl_cam = torch.gather(out_smpl_cam, 1, topk_smpl[:, :, None].repeat(1, 1, 3))
        smpl_kp3d = torch.gather(out_smpl_kp3d, 1, topk_smpl[:, :, None, None].repeat(1, 1, out_smpl_kp3d.shape[-2], 3))
        smpl_verts = torch.gather(out_smpl_verts, 1,
                                  topk_smpl[:, :, None, None].repeat(1, 1, out_smpl_verts.shape[-2], 3))
        ...
        for bs in range(batch_size):
            ...
            if 'bb2img_trans' in data_batch_nc:
                results[-1]['bb2img_trans'] = data_batch_nc['bb2img_trans'][bs]

            if 'bb2img_trans' in data_batch_nc:
                results[-1]['img2bb_trans'] = data_batch_nc['img2bb_trans'][bs]

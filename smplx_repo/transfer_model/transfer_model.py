...
def run_fitting(
    ...
    transl = var_dict['transl'].clone().detach()
    # param_dict['transl'] = var_dict['transl'] * 0.
    # param_dict['transl'] = transl
    body_model_output = body_model(
        return_full_pose=True, get_skin=True, **param_dict)
    var_dict.update(body_model_output)
    var_dict['faces'] = body_model.faces
    # var_dict['vertices'] = var_dict['vertices'] + transl[:,None]
    # var_dict['joints'] = var_dict['joints'] + transl[:,None]
    var_dict['transl'] = transl
